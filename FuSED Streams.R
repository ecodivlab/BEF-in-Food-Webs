...
for(i in 1:length(web)){
  ...
  meta.IS$biomass[i] <- sum(attributes$biomass, na.rm=T)
  meta.IS$animal.biomass[i] <- sum(attributes$biomass[attributes$species_type=="animal"], na.rm=T)
  meta.IS$plant.biomass[i] <- sum(attributes$biomass[attributes$species_type=="plant"], na.rm=T)
  ...
  meta.IS$generality[i] <- Gen.sd(bin.matrix)
  meta.IS$vulnerability[i] <- Vul.sd(bin.matrix[!basals, !basals])
  meta.IS$MeanTL[i] <- mean(attributes$TL)
  meta.IS$MaxTL[i] <- max(attributes$TL)
  #meta.IS$modularity[i] <- cluster_spinglass(igraph)$modularity
  ...
  meta.IS$prim.cons.richness[i] = sum(primary.cons.and.omnivores) # primary consumer richness
  meta.IS$sec.cons.richness[i] = sum(sec.cons)  # secondary consumer richness
}
...
for(i in 1:length(web)){
  ...
  temperature = meta.UK$temperature[meta.UK$FW_name == web[i]]
  ...
  attributes$TL <- TL(matrix)
  attributes$bodymass <- (attributes$bodymass*4) #Convert body mass dry to fresh mass conversion
  ...
  attributes$biomass[web[i]=='Afon Marteg'] <- (log(attributes$biomass) + abs(min(log(attributes$biomass)))) +1
  ...
  meta.UK$biomass[i] <- sum(attributes$biomass, na.rm=T)
  meta.UK$animal.biomass[i] <- sum(attributes$biomass[attributes$species_type=="animal"], na.rm=T)
  meta.UK$plant.biomass[i] <- sum(attributes$biomass[attributes$species_type=="plant"], na.rm=T)
  ...
  meta.UK$generality[i] <- Gen.sd(bin.matrix)
  meta.UK$vulnerability[i] <- Vul.sd(bin.matrix[!basals, !basals])
  meta.UK$MeanTL[i] <- mean(attributes$TL)
  meta.UK$MaxTL[i] <- max(attributes$TL)
  #meta.UK$modularity[i] <- cluster_spinglass(igraph)$modularity
  ...
  meta.UK$prim.cons.richness[i] = sum(primary.cons.and.omnivores) # primary consumer richness
  meta.UK$sec.cons.richness[i] = sum(sec.cons)  # secondary consumer richness
}


#### River data from Mario Brauns #### 
...
for(i in 1:length(web)){
  ...
  temperature = meta.Brauns$temperature
  temp.kT = temperature + 273.15
  temp.arr = ((273.15+temperature)-T0)/(boltz*(273.15+temperature)*T0)
  ...
  ## Biomass distributions
  meta.Brauns$biomass[i] <- sum(attributes$biomass, na.rm=T)
  meta.Brauns$animal.biomass[i] <- sum(attributes$biomass[animals], na.rm=T)
  meta.Brauns$plant.biomass[i] <- sum(attributes$biomass[plants], na.rm=T)
  ...
  meta.Brauns$generality[i] <- Gen.sd(bin.matrix)
  meta.Brauns$vulnerability[i] <- Vul.sd(bin.matrix)
  meta.Brauns$MeanTL[i] <- mean(attributes$TL)
  meta.Brauns$MaxTL[i] <- max(attributes$TL)
  #meta.Brauns$modularity[i] <- cluster_spinglass(igraph)$modularity
  ...
  meta.Brauns$prim.cons.richness[i] = sum(primary.cons.and.omnivores) # primary consumer richness
  meta.Brauns$sec.cons.richness[i] = sum(sec.cons)  # secondary consumer richness
}
...
for(i in 1:length(web)){
  ...
  temperature = meta.Brazil$temperature[meta.Brazil$FW_name == web[i]]
  temp.kT = temperature + 273.15
  temp.arr = ((273.15+temperature)-T0)/(boltz*(273.15+temperature)*T0)
  ...
  meta.Brazil$biomass[i] <- sum(attributes$biomass, na.rm=T)
  meta.Brazil$animal.biomass[i] <- sum(attributes$biomass[attributes$metabolic_type=="animal"], na.rm=T)
  meta.Brazil$plant.biomass[i] <- sum(attributes$biomass[attributes$metabolic_type=="plant"], na.rm=T)
  ...
  meta.Brazil$generality[i] <- Gen.sd(bin.matrix)
  meta.Brazil$vulnerability[i] <- Vul.sd(bin.matrix[!basals, !basals])
  meta.Brazil$MeanTL[i] <- mean(attributes$TL)
  meta.Brazil$MaxTL[i] <- max(attributes$TL)
  #meta.Brazil$modularity[i] <- cluster_spinglass(igraph)$modularity
  ...
  meta.Brazil$prim.cons.richness[i] = sum(primary.cons.and.omnivores) # primary consumer richness
  meta.Brazil$sec.cons.richness[i] = sum(sec.cons)  # secondary consumer richness
}
