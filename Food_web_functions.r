...
Plot.matrix <- function(web, title=" ", point.cex=0.5, trait.cex=1, xlab="Consumer", ylab="Resource",
                        diag.line=T, traits=F, by.consumer=T, axes.labels=F, sp.pt.ch=NA, pt.col="black"){
    
  S <- length(web[,1])
  web <- t(web)  ## Note: Added this line of code to fix follow-on problem with Matrix.to.list (see below)
  ...
  if(axes.labels){
      mtext(side=2, ylab, line=0.5, cex=1)
      mtext(side=3, xlab, line=0.5, cex=1)
  }
  if(diag.line==T)
...
Matrix.to.list <- function(web.matrix, predator.first=TRUE){
  ...
  for(i in 1:length(web.matrix[,1]))
    for(j in 1:length(web.matrix[,1]))
      if(web.matrix[j,i]==1){   ##Note: swapped i and j to fix problem with order of predator/prey columns
...
## OWen Petchey's trophic level function
GetTL <- function(web){
...
## Compute the average food chain length of the top species
average.chain.length = function(graph.mat){
  require(igraph)
  igraph = graph_from_adjacency_matrix(graph.mat, mode="directed")
  tops = which(rowSums(graph.mat) == 0)
  x = list()
  for (node in tops){
    x = c(x, all_simple_paths(igraph, from=node, mode="in"))
  }
  tot.chains = length(x)
  mean.chain = 0
  #vec.chains = c()
  for (elem in x){
    mean.chain = mean.chain + length(elem)
    #vec.chains = c(vec.chains, length(elem)-1)
  }
  mean.chain = mean.chain / tot.chains - 1
  return(mean.chain)
}
