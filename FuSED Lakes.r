...
for(i in 1:length(lakes)) {
  ...
  # temperature
    temp <- meta$temperature_C[meta$FW_name==lake]
  ...
  # efficiencies
    nodes$efficiencies[nodes$metabolic.type=="invertebrate" | nodes$metabolic.type=="ectotherm vertebrate"] <- exp(2.266)*exp(0.164*temp.kT) / (1 + exp(2.266)*exp(0.164*temp.kT)) #animal
    nodes$efficiencies[nodes$metabolic.type=="primary producer"] <- exp(0.179)*exp(0.164*temp.kT) / (1 + exp(0.179)*exp(0.164*temp.kT)) # plant
    nodes$efficiencies[nodes$metabolic.type=="detritus"] <- exp(-1.670)*exp(0.164*temp.kT) / (1 + exp(-1.670)*exp(0.164*temp.kT)) #detritus
  ...
  #                 bioms.losses=F, bioms.prefs = F) * perday #* 1000000 #*1000000 to convert from ml to cubic meters
  ...
  meta$animal.biomass[i] <- sum(attributes$biomass[attributes$metabolic.type=="invertebrate" | attributes$metabolic.type=="ectotherm vertebrate"], na.rm=T)
  meta$plant.biomass[i] <- sum(attributes$biomass[attributes$metabolic.type=="primary producer"], na.rm=T)
  ...
  meta$prim.cons.richness[i] = sum(primary.cons.and.omnivores) # primary consumer richness
  meta$sec.cons.richness[i] = sum(sec.cons)  # secondary consumer richness
}
#view(meta[,-c(2:28, 30:32, 34)])

meta.Lakes <- meta[,-c(2:28, 30:32, 34)]
